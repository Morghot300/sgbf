...
-- trg_affectation_mission_immutabilite (RG-MIS-006, V4) interdit toute
-- modification de agent_id APRES creation - protection legitime contre une
-- reecriture applicative de l'identite d'une affectation, mais qui bloquerait
-- aussi ce repointage structurel one-shot (meme affectation reelle, seule sa
-- reference numerique change de table source). Deposee puis recreee a
-- l'identique juste apres : aucun changement de comportement pour
-- l'application au-dela de cette migration.
DROP TRIGGER trg_affectation_mission_immutabilite;
